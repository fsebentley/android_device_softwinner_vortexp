...
#define SENSORS_SUPPORT_COUNT		1//3
...
static int control_open_input(int mode)
{
	...
	*filename++ = '/';
	while ((de = readdir(dir))) {
		...
		if (fd >= 0) {
			...
			if (!strcmp(name, "ecompass_data")) {
				ALOGD("Using %s (name=%s)", devname, name);
				break;
			}
			close(fd);
			fd = -1;
	...
	if (fd < 0) {
		ALOGD("Couldn't find or open 'compass' driver (%s)", strerror(errno));
	}
...
static int control_open_ecs(struct sensors_poll_context_t* dev)
{
	if (dev->ecs_fd < 0) {
		...
		ALOGE_IF(dev->ecs_fd < 0, "Couldn't open %s (%s)",
...
static void control_enable_disable_sensors(int fd, uint32_t sensors, uint32_t mask)
{
	...
	if (mask & SENSORS_ACCELERATION) {
		...
		if (ioctl(fd, ECOMPASS_IOC_SET_AFLAG, &flags) < 0) {
			ALOGE("ECOMPASS_IOC_SET_AFLAG error (%s)", 
				strerror(errno));
		}
	}
	if (mask & SENSORS_MAGNETIC_FIELD) {
		...
		if (ioctl(fd, ECOMPASS_IOC_SET_MFLAG, &flags) < 0) {
			ALOGE("ECOMPASS_IOC_SET_MFLAG error (%s)", 
				strerror(errno));
		}
	}
	if (mask & SENSORS_ORIENTATION) {
		...
		if (ioctl(fd, ECOMPASS_IOC_SET_OFLAG, &flags) < 0) {
			ALOGE("ECOMPASS_IOC_SET_OFLAG error (%s)", 
				strerror(errno));
		}
	}
}
    ...
    sensors_event_t* data, int count)
{
	...
	while (mask) {
		...
		if (dev->pending_sensors & (1 << i)) {
			...
			ALOGD_IF(0, "%s: %d-%d [%f, %f, %f], sensor: %d, type: %d", 
				__FUNCTION__, num, i,
	...
	if (num == 0) {
		ALOGE("No sensor to return! pending_sensors=%08x", dev->pending_sensors);
			...
			int handle, int enabled)
{
	...
	ALOGD("Active sensors=%08x, real=%08x",
	...
	int handle, int64_t ns)
{
	struct sensors_poll_context_t *dev;
	dev = (struct sensors_poll_context_t *)device;
	ALOGD("+%s: ns=%d", __FUNCTION__, (int)ns);
	...
	sensors_event_t* data, int count)
{
	...
	if (fd < 0) {
		ALOGE("Invalid file descriptor, fd=%d", fd);
		return -1;
	}
	if (data == NULL || count < 0) {
		ALOGE("Invalid argument count=%d", count);
		return -1;
	}
	ALOGD_IF(0, "%s, sensors_event: %p, count: %d",
	...
	while (1) {
		...
		uint32_t v;
		ALOGD_IF(0, "type: %d code: %d value: %-5d time: %ds-%dns",
		...
		if (event.type == EV_ABS) {
			...
			if (event.code == SYN_CONFIG) {
				...
				// main loop.
				//ALOGD("got empty message: value=%d", event.value);
				return 0x7FFFFFFF;
			}
			if (new_sensors) {
